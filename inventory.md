# Game Card Inventory & Monster Requirements

This document defines the current card inventory, weapon distribution, Monster requirements, and scoring for the base game.

---

# Player Draw Deck

The game uses one shared Draw deck containing **24 cards**.

## Standard Weapons

There are **3 copies of each standard Weapon card**.

| Weapon  | Copies |
| ------- | -----: |
| Bow     |      3 |
| Grenade |      3 |
| Mace    |      3 |
| Sword   |      3 |
| Spear   |      3 |
| Gun     |      3 |

**Total standard Weapon cards: 18**

## Draw Cards

| Card   | Copies |
| ------ | -----: |
| Draw 1 |      3 |
| Draw 2 |      2 |

**Total Draw cards: 5**

## Special Weapon

| Card       | Copies |
| ---------- | -----: |
| Black Hole |      1 |

The Black Hole can defeat any normal Monster by itself.

The Black Hole is removed from play during Sudden Death.

## Total Shared Draw Deck

* 18 standard Weapon cards
* 3 Draw 1 cards
* 2 Draw 2 cards
* 1 Black Hole

**Total: 24 cards**

---

# Monster Deck

There is **1 copy of each Monster**.

The normal game contains **16 Monsters**.

## 1-Point Monsters

Each 1-point Monster requires **2 Weapons**.

| Monster        | Required Weapons | Points |
| -------------- | ---------------- | -----: |
| Socket         | Gun + Spear      |      1 |
| Top            | Grenade + Sword  |      1 |
| Bitty Bitey    | Mace + Spear     |      1 |
| Flower Trap    | Mace + Bow       |      1 |
| Smasher        | Bow + Mace       |      1 |
| Wrecking Snake | Spear + Sword    |      1 |
| Boom Boom      | Spear + Gun      |      1 |
| Rocket Chopper | Grenade + Spear  |      1 |
| Wacker         | Mace + Sword     |      1 |
| Chomper        | Grenade + Sword  |      1 |

**1-point Monsters: 10**

**Total available points: 10**

---

# 2-Point Monsters

Each 2-point Monster requires **3 Weapons**.

| Monster     | Required Weapons        | Points |
| ----------- | ----------------------- | -----: |
| Pincher     | Grenade + Spear + Sword |      2 |
| Rock Crab   | Grenade + Gun + Bow     |      2 |
| Ground Worm | Grenade + Gun + Mace    |      2 |
| Spikey      | Bow + Sword + Spear     |      2 |
| Kraken      | Bow + Gun + Grenade     |      2 |

**2-point Monsters: 5**

**Total available points: 10**

---

# 3-Point Monster

The 3-point Monster requires **4 Weapons**.

| Monster | Required Weapons            | Points |
| ------- | --------------------------- | -----: |
| Thing   | Gun + Spear + Grenade + Bow |      3 |

**3-point Monsters: 1**

**Total available points: 3**

---

# Normal Game Totals

The regular Monster deck contains:

* 10 × 1-point Monsters
* 5 × 2-point Monsters
* 1 × 3-point Monster

**Total Monsters: 16**

**Total available points: 23**

Because 23 is an odd number, not every game can result in a tied score. However, ties between two or more players remain possible depending on how the Monsters are divided.

---

# Sudden Death Monster

The Infinity Beast is a dedicated Monster and is **not shuffled into the normal Monster deck**.

| Monster            | Required Weapons      | Value              |
| ------------------ | --------------------- | ------------------ |
| The Infinity Beast | Grenade + Sword + Gun | Infinite / Victory |

## Infinity Beast Rules

The Infinity Beast appears only when two or more players are tied for the highest score after all normal Monsters have been defeated.

During Sudden Death:

* Only tied players participate.
* The Black Hole is removed from the game.
* Maximum hand size is reduced from 5 cards to 4 cards.
* The Infinity Beast requires Grenade + Sword + Gun.
* The first player to defeat The Infinity Beast immediately wins.

The Infinity Beast does not actually add points to a player's normal score. Its **Infinite Points** value represents an immediate victory.

---

# Card Counts at a Glance

## Shared Draw Deck

| Type       |  Cards |
| ---------- | -----: |
| Bow        |      3 |
| Grenade    |      3 |
| Mace       |      3 |
| Sword      |      3 |
| Spear      |      3 |
| Gun        |      3 |
| Draw 1     |      3 |
| Draw 2     |      2 |
| Black Hole |      1 |
| **Total**  | **24** |

## Monster Cards

| Type                             |  Cards |
| -------------------------------- | -----: |
| 1-point Monsters                 |     10 |
| 2-point Monsters                 |      5 |
| 3-point Monsters                 |      1 |
| Infinity Beast                   |      1 |
| **Total physical Monster cards** | **17** |

Of those 17 Monster cards, **16 are used during the normal game** and **1 is reserved for Sudden Death**.
